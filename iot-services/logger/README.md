...
`bouncy-logger` is a very small HTTP logging service for `pi-nas`.

It accepts HTTP `PUT` requests from LAN devices, checks a shared secret key, and appends the request body to a log file stored on the NAS RAID volume, not on the Raspberry Pi SD card.

The original target client is an Arduino or Arduino-like device that periodically sends a short string to be logged.
...
```text
/opt/pi-nas-services/bouncy-logger -> /home/joe/workspace/bouncy-logger
```
...
```text
/home/joe/workspace/bouncy-logger
```
...
```bash
sudo diff -u \
  /home/joe/workspace/bouncy-logger/docker-compose.yml \
  /srv/dev-disk-by-uuid-1b03c883-24b8-4bbc-9e3f-9fdc70389470/pi-nas/bouncy-logger/bouncy-logger.yml
```
...
```bash
sudo cmp -s \
  /home/joe/workspace/bouncy-logger/.env \
  /srv/dev-disk-by-uuid-1b03c883-24b8-4bbc-9e3f-9fdc70389470/pi-nas/bouncy-logger/bouncy-logger.env \
...
```bash
repo_key_hash="$(
  sed -n 's/^SHARED_KEY=//p' /home/joe/workspace/bouncy-logger/.env |
  sha256sum |
  awk '{print $1}'
)"
